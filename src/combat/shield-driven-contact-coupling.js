...
export const SHIELD_CONTACT_COUPLING_PROFILES = Object.freeze({
  block: Object.freeze({
    ...
    holdMs: 18,
    driveEndMs: 68,
    shieldGiveMeters: 0.035,
    shieldSweepMeters: 0,
    shieldLiftMeters: 0,
    attackerFollowRatio: 0.82,
    attackerReleaseBiasMeters: 0.012,
    defenderUpperArmMaxDegrees: 10,
    defenderLowerArmMaxDegrees: 14,
    attackerUpperArmMaxDegrees: 10,
    attackerLowerArmMaxDegrees: 14,
  }),
  parry: Object.freeze({
    outcome: 'parry',
    durationMs: 96,
    holdMs: 12,
    driveEndMs: 72,
    shieldGiveMeters: 0.012,
    shieldSweepMeters: 0.105,
    shieldLiftMeters: 0.028,
    attackerFollowRatio: 0.88,
    attackerReleaseBiasMeters: 0.018,
    defenderUpperArmMaxDegrees: 16,
    defenderLowerArmMaxDegrees: 22,
    attackerUpperArmMaxDegrees: 14,
    attackerLowerArmMaxDegrees: 20,
  }),
  'perfect-parry': Object.freeze({
    ...
    holdMs: 10,
    driveEndMs: 76,
    shieldGiveMeters: 0.010,
    shieldSweepMeters: 0.125,
    shieldLiftMeters: 0.036,
    attackerFollowRatio: 0.92,
    attackerReleaseBiasMeters: 0.024,
    defenderUpperArmMaxDegrees: 18,
    defenderLowerArmMaxDegrees: 25,
    attackerUpperArmMaxDegrees: 16,
    attackerLowerArmMaxDegrees: 23,
  }),
});
...
function resolveOutcome(value) {
  const outcome = String(value || 'block').toLowerCase();
  return SHIELD_CONTACT_COUPLING_PROFILES[outcome] ? outcome : 'block';
}

function lateralSignFor(direction, contactPoint = {}) {
  if (direction === 'left') return 1;
  if (direction === 'right') return -1;
  const x = finite(contactPoint.x);
  return Math.abs(x) > 0.015 ? Math.sign(x) : 1;
}

export function getShieldContactCouplingProfile(outcome = 'block', overrides = {}) {
  const base = SHIELD_CONTACT_COUPLING_PROFILES[resolveOutcome(outcome)];
...
export function sampleShieldContactCoupling(input = {}) {
  ...
  const incomingDirection = normalize(vec(input.incomingVelocity || input.incomingDirection), { x: 0, y: 0, z: 1 });
  ...
  if (elapsedMs >= profile.durationMs) {
    return Object.freeze({
      ...
      outcome,
      phase: SHIELD_CONTACT_COUPLING_PHASES.COMPLETE,
      elapsedMs,
      complete: true,
      releaseAttackerRecoil: true,
      shieldOffset: freezeVector({ x: 0, y: 0, z: 0 }),
      attackerWeaponOffset: freezeVector({ x: 0, y: 0, z: 0 }),
      profile,
    });
  }
  ...
  if (elapsedMs > profile.holdMs && elapsedMs <= profile.driveEndMs) {
    ...
    phase = SHIELD_CONTACT_COUPLING_PHASES.RELEASE;
    drive = 1;
    release = smoothstep((elapsedMs - profile.driveEndMs) / Math.max(1, profile.durationMs - profile.driveEndMs));
  }

  const givePeak = profile.shieldGiveMeters * (phase === SHIELD_CONTACT_COUPLING_PHASES.RELEASE ? 1 - 0.45 * release : drive);
  const sweepPeak = profile.shieldSweepMeters * drive;
  const liftPeak = profile.shieldLiftMeters * drive;
  const shieldOffset = add(
    add(scale(incomingDirection, givePeak), scale(tangent, sweepPeak)),
    scale(worldUp, liftPeak),
  );

  const follow = scale(shieldOffset, profile.attackerFollowRatio);
  const releaseBias = phase === SHIELD_CONTACT_COUPLING_PHASES.RELEASE
    ? profile.attackerReleaseBiasMeters * release
    : 0;
  const attackerWeaponOffset = add(follow, scale(tangent, releaseBias));
...
function aimEffectorWithBone(THREE, bone, effectorWorld, targetWorld, maxDegrees) {
  ...
  const rawAngle = 2 * Math.acos(Math.max(-1, Math.min(1, Math.abs(desiredWorldDelta.w))));
...
export function createShieldDrivenContactCouplingRuntime(THREE, options = {}) {
  if (!THREE?.Vector3 || !THREE?.Quaternion) {
    throw new Error('G4.3B.5R.2 requires THREE.Vector3 + Quaternion');
  }
  ...
  if (!defenderRig?.bones?.['upperarm.l'] || !defenderRig?.bones?.['lowerarm.l']) {
    throw new Error('G4.3B.5R.2 requires defender left-arm bones');
  }
  if (!attackerRig?.bones?.['upperarm.r'] || !attackerRig?.bones?.['lowerarm.r'] || !attackerRig?.bones?.['hand.r']) {
    throw new Error('G4.3B.5R.2 requires attacker right-arm bones');
  }
  ...
  const defenderEffector = new THREE.Vector3();
  const attackerBaseHand = new THREE.Vector3();
  const attackerTarget = new THREE.Vector3();
  const attackerEffector = new THREE.Vector3();
  let active = null;
  let lastReport = null;
  ...
  function start(input = {}) {
    ...
    attackerRig.bones['hand.r'].getWorldPosition(attackerBaseHand);
    active = {
      outcome,
      profile,
      attackDirection: String(input.attackDirection || ''),
  ...
  function update(deltaSeconds = 1 / 60) {
    if (!active) return Object.freeze({ active: false, stage: SHIELD_DRIVEN_CONTACT_COUPLING_STAGE });
    ...
    const sample = sampleShieldContactCoupling({
      ...
      contactPoint: active.contactPoint,
    ...
    if (!sample.complete) {
      ...
      for (let iteration = 0; iteration < 2; iteration += 1) {
        ...
        defenderEffector.set(surface.center.x, surface.center.y, surface.center.z);
        ...
        defenderEffector.set(surfaceAfterLower.center.x, surfaceAfterLower.center.y, surfaceAfterLower.center.z);
      ...
      attackerTarget.set(
        attackerBaseHand.x + sample.attackerWeaponOffset.x,
        attackerBaseHand.y + sample.attackerWeaponOffset.y,
        attackerBaseHand.z + sample.attackerWeaponOffset.z,
      );
      ...
      appliedDegrees.attackerLowerArm = aimEffectorWithBone(
        ...
        attackerEffector,
        attackerTarget,
        active.profile.attackerLowerArmMaxDegrees,
      );
      attackerRig.root?.updateMatrixWorld?.(true);
      attackerRig.bones['hand.r'].getWorldPosition(attackerEffector);
      appliedDegrees.attackerUpperArm = aimEffectorWithBone(
        ...
        attackerEffector,
        attackerTarget,
        active.profile.attackerUpperArmMaxDegrees,
      );
      attackerRig.root?.updateMatrixWorld?.(true);
    }
