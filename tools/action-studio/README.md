...
- `studio-external-animation-controller.js` — KayKit loading plus UAL2 sword-clip retarget, preview, fit and JSON binding.

Action motion is driven by `src/animation/action-motion-player.js`. Every action owns a normalized `animationBinding`: `authored` uses Action Studio pose keys, while `kaykit` and `ual2` reference a clip by name and deterministically map the action frame to animation time. The JSON never embeds a Three.js `AnimationClip` or GLB data.
...
The generated `index.html` deliberately loads a classic script so the pose editor can open directly through `file://` without browser ES-module CORS errors. Loading either external animation library requires serving the repository through local HTTP.
