...
const buckler = createProceduralBuckler(THREE, { ...ACCEPTED_OFFHAND_BUCKLER_SHAPE_G423, lineMode: true, solidVisible: false });
...
const couplingRuntime = createShieldDrivenContactCouplingRuntime(THREE, { defenderRig: defender.rig, attackerRig: attacker.rig, buckler });
...
const combat = createTwoActorCombatIntegration({
  ...
  attackRuntime,
  guardMachine,
  parrySync: { presentationOffsetSeconds: 0.35, parryAttackerRecoilDelayMs: 0, perfectParryAttackerRecoilDelayMs: 0 },
...
function updatePreContact(snapshot, currentBlade, deltaSeconds) {
  ...
  if (selectedMode === 'block') {
    ...
    latestTrackingReport = trackingRuntime.update(latestTrackingPlan, deltaSeconds); defender.update(0, camera); defenderSword?.update(); return;
  }
  ...
  if (latestAnalysis?.threat?.point) {
    predictedMarker.position.set(latestAnalysis.threat.point.x, latestAnalysis.threat.point.y, latestAnalysis.threat.point.z); predictedMarker.visible = true;
  }
  ...
  latestTrackingReport = trackingRuntime.update(latestTrackingPlan, deltaSeconds); defender.update(0, camera); defenderSword?.update();
}
function resolveContact(snapshot, currentBlade, deltaSeconds) {
  ...
  couplingRuntime.start({ outcome: latestCombatResult.resolution.outcome, attackDirection: latestCombatResult.resolution.attackDirection, contact: latestContact, surfaceAtContact });
...
function updateCoupling(deltaSeconds) {
  ...
  if (latestCouplingReport?.finalSurface?.center) {
    const p = latestCouplingReport.finalSurface.center; driveMarker.position.set(p.x, p.y, p.z); driveMarker.visible = true;
  }
