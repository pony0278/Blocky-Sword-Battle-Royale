...
function resetToHold() {
  machine.send(GUARD_EVENTS.RESET, { verification: 'g351pt33-reset' });
  runtime.sync();
  machine.send(GUARD_EVENTS.GUARD_PRESS, { verification: 'g351pt33-guard-press' });
  runtime.sync();
...
function beginParry(perfect) {
  ...
  const result = machine.send(GUARD_EVENTS.PARRY_CONFIRMED, {
    verification: perfect ? 'g351pt33-perfect' : 'g351pt33-parry',
    perfect,
  });
...
function measureVariant(perfect) {
  ...
  const spinePass = metrics.spine.maxStepDegrees <= 12
    && metrics.spine.maxExcursionDegrees <= 45
    && metrics.spine.cumulativeTravelDegrees <= 150;
  const chestPass = metrics.chest.maxStepDegrees <= 12
    && metrics.chest.maxExcursionDegrees <= 55
    && metrics.chest.cumulativeTravelDegrees <= 160;
  return {
    variant,
    metrics,
    rootPass,
...
async function main() {
  status.textContent = `${PARRY_UPPER_BODY_CONTINUITY_STAGE} loading production Skyrim Guard clips…`;
  ...
  const parryUpperPolicy = parryClip?.userData?.productionParryDeflect?.upperBodyContinuity || null;
  const perfectUpperPolicy = perfectClip?.userData?.productionParryDeflect?.upperBodyContinuity || null;
  ...
  const upperPolicyPass = [parryUpperPolicy, perfectUpperPolicy].every((policy) => (
    ...
    && policy?.policy === 'contact-relative-clamped-torso-deflect'
  ...
  const pass = policyPass && upperPolicyPass && parry.pass && perfect.pass;
  const report = {
    ...
    thresholds: {
      ...
      spine: { maxStepDegrees: 12, maxExcursionDegrees: 45, cumulativeTravelDegrees: 150 },
      chest: { maxStepDegrees: 12, maxExcursionDegrees: 55, cumulativeTravelDegrees: 160 },
    },
  };
  ...
  status.textContent = `${PARRY_UPPER_BODY_CONTINUITY_STAGE} ${pass ? 'PASS' : 'FAIL'} · 0–599ms @ 60fps root/hips/spine/chest continuity`;
  status.className = pass ? 'good' : 'bad';
  window.__G351PT33_RESULT__ = report;
}

main().catch((error) => {
  document.documentElement.dataset.g351pt32 = 'fail';
  document.documentElement.dataset.g351pt33 = 'fail';
  status.textContent = `${PARRY_UPPER_BODY_CONTINUITY_STAGE} FAIL · ${error?.message || error}`;
  status.className = 'bad';
  reportNode.textContent = error?.stack || String(error);
  window.__G351PT33_RESULT__ = { stage: PARRY_UPPER_BODY_CONTINUITY_STAGE, pass: false, error: error?.stack || String(error) };
});
